...
const getProductsByCategoryId = async (req, res, next) => {
    ...
    try {
        categoryWithProducts = await Category.findById(cid).populate(
            'products'
        );
    } catch (err) {
        const error = new HttpError(
            'something went wrong on DB, when finding the given category ID',
            500
        );
        return next(error);
    }
    ...
    if (!categoryWithProducts || categoryWithProducts.products.length === 0) {
        return next(
            new HttpError('no products found for the given category id', 404)
        );
    }
...
const createProduct = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError(
            'something went wrong on the db side when trying to retrive the related category for the product from the db',
            500
        );
        return next(error);
    }
...
const deleteProduct = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError(
            'something went wrong on db side, when finding the given product id'
        );
        return next(error);
    }
    ...
    if (!selectedProduct) {
        return next(
            new HttpError('there is no record for the given product id', 404)
        );
    }
